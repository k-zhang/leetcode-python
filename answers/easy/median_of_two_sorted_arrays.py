...
class Solution:
    def find_median_sorted_arrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        if total_length % 2 == 1:
            return self.find_kth_element(nums1, nums2, total_length // 2 + 1)
        else:
            return (self.find_kth_element(nums1, nums2, total_length // 2) + self.find_kth_element(nums1, nums2, total_length // 2 + 1)) / 2
